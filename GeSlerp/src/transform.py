...
class CachedLabelPointerTransformWithPromptReplace(CachedTransformOneBase):
    """Transform for label-token linking for skip consecutive spans"""
        ...
        self.collate_fn: GeneralCollateFn = GeneralCollateFn(
            {
                ...
                "labels": torch.long,
                "spans": None,
                "prompts_to_indices":torch.long,
                "prompts_ids":torch.long,
                "prompts_mask":torch.long,
            },
            guessing=False,
            missing_key_as_null=True,
            # only for pre-training
    ...
    def transform(self, instance: dict, **kwargs):
        ...
        if 'ent' in descriptions.keys():
            for idx, (entity, description) in enumerate(descriptions['ent'].items()):
                dec = self.tokenizer(description, add_special_tokens=True)
                prompts_ids.append(dec['input_ids'])
                prompts_mask.append(dec['attention_mask'])
                prompts_to_indices.append([idx] + [label_map["lm"][entity][0]])
                if len(label_map["lm"][entity])>1:
                    print(label_map["lm"][entity])
                    
        if 'rel' in descriptions.keys():
            for idx, (relation, description) in enumerate(descriptions['rel'].items()):
                dec = self.tokenizer(description, add_special_tokens=True)
                prompts_ids.append(dec['input_ids'])
                prompts_mask.append(dec['attention_mask'])
                prompts_to_indices.append([idx] + [label_map["lr"][relation][0]])
                if len(label_map["lr"][relation])>1:
                    print(label_map["lr"][relation])
        if 'discontinuous_ent' in descriptions.keys():
            for idx, (entity, description) in enumerate(descriptions['discontinuous_ent'].items()):
                dec = self.tokenizer(description, add_special_tokens=True)
                prompts_ids.append(dec['input_ids'])
                prompts_mask.append(dec['attention_mask'])
                prompts_to_indices.append([idx] + [label_map["lm"][entity][0]])
        ...
        if 'event' in descriptions.keys():
            for idx,(trigger,event) in enumerate(descriptions['event'].items()):
                count = 0
                for jdx,(entity,description) in enumerate(event.items()):
                    if count == 0:
                        dec = self.tokenizer(description, add_special_tokens=True)
                        prompts_ids.append(dec['input_ids'])
                        prompts_mask.append(dec['attention_mask'])
                        ...
                        prompts_ids.append(dec['input_ids'])
                        prompts_mask.append(dec['attention_mask'])
                        prompts_to_indices.append([idx] + [label_map["lr"][entity][0]])
                        count+=1
                    
        if 'cls' in descriptions.keys():
            for idx, (entity, description) in enumerate(descriptions['cls'].items()):
                dec = self.tokenizer(description, add_special_tokens=True)
                prompts_ids.append(dec['input_ids'])
                prompts_mask.append(dec['attention_mask'])
                prompts_to_indices.append([idx] + [label_map["lc"][entity][0]])
                if len(label_map["lc"][entity])>1:
                    print(label_map["lc"][entity])

        if 'hyper_rel' in descriptions.keys():
            for idx,(trigger,relation) in enumerate(descriptions['hyper_rel'].items()):
                count = 0
                for jdx,(rel,description) in enumerate(relation.items()):
                    if count == 0:
                        dec = self.tokenizer(description, add_special_tokens=True)
                        prompts_ids.append(dec['input_ids'])
                        prompts_mask.append(dec['attention_mask'])
                        prompts_to_indices.append([idx] + [label_map["lr"][rel][0]])
                        count+=1
                    else:
                        dec = self.tokenizer(description, add_special_tokens=True)
                        prompts_ids.append(dec['input_ids'])
    ...
    def skip_consecutive_span_labels(self, data: dict) -> dict:
        bs = len(data["input_ids"])
        des_len = len(data["prompts_ids"][0])
        max_seq_len = max(len(input_ids) for input_ids in data["input_ids"])
        max_prompt_len = max(len(prompts_ids) for prompts_ids in data["prompts_ids"][0])
        # batch_seq_len = min(self.max_seq_len, max_seq_len)
        batch_seq_len = min(self.max_seq_len, max_seq_len)
        batch_prompt_len = min(self.max_seq_len, max_prompt_len)
        for i in range(bs):
            data["input_ids"][i] = data["input_ids"][i][:batch_seq_len]
            data["mask"][i] = data["mask"][i][:batch_seq_len]
            assert len(data["input_ids"][i]) == len(data["mask"][i])
            ...
            for j in range(des_len):
                data["prompts_ids"][i][j] = data["prompts_ids"][i][j][:batch_prompt_len]
                data["prompts_mask"][i][j] = data["prompts_mask"][i][j][:batch_prompt_len]
                assert len(data["prompts_ids"][i]) == len(data["prompts_mask"][i])
                pad_len = batch_prompt_len - len(data["prompts_mask"][i][j])
                data["prompts_ids"][i][j] = (
                data["prompts_ids"][i][j] + [self.tokenizer.pad_token_id] * pad_len
                )
                data["prompts_mask"][i][j] = data["prompts_mask"][i][j] + [0] * pad_len
            # # for debugging only
            # pred_spans = decode_nnw_nsw_thw_mat(data["labels"][i].unsqueeze(0))[0]
            # sorted_gold = sorted(set(tuple(x) for x in data["spans"][i]))
            # sorted_pred = sorted(set(tuple(x) for x in pred_spans))
...
class CachedLabelPointerTransformWith_PN_PromptReplace(CachedTransformOneBase):
    """Transform for label-token linking for skip consecutive spans"""
        ...
        self.collate_fn: GeneralCollateFn = GeneralCollateFn(
            {
                ...
                "prompts_ids":torch.long,
                "prompts_mask":torch.long,
                "prompts_to_indices_neg":torch.long,
                "prompts_ids_neg":torch.long,
                "prompts_mask_neg":torch.long,
            },
            guessing=False,
            missing_key_as_null=True,
            # only for pre-training
    ...
    def transform(self, instance: dict, **kwargs):
        ...
        prompts_to_indices_neg=[]
        prompts_ids_neg = []
        prompts_mask_neg = []
        
        descriptions_neg = instance['description_neg']
        
        if 'ent' in descriptions_neg.keys():
            for idx, (entity, description) in enumerate(descriptions_neg['ent'].items()):
                dec = self.tokenizer(description, add_special_tokens=True)
                prompts_ids_neg.append(dec['input_ids'])
                prompts_mask_neg.append(dec['attention_mask'])
                prompts_to_indices_neg.append([idx] + [label_map["lm"][entity][0]])
                if len(label_map["lm"][entity])>1:
                    print(label_map["lm"][entity])
                    
        if 'rel' in descriptions_neg.keys():
            for idx, (relation, description) in enumerate(descriptions_neg['rel'].items()):
                dec = self.tokenizer(description, add_special_tokens=True)
                prompts_ids_neg.append(dec['input_ids'])
                prompts_mask_neg.append(dec['attention_mask'])
                prompts_to_indices_neg.append([idx] + [label_map["lr"][relation][0]])
                if len(label_map["lr"][relation])>1:
                    print(label_map["lr"][relation])
        if 'discontinuous_ent' in descriptions_neg.keys():
            for idx, (entity, description) in enumerate(descriptions_neg['discontinuous_ent'].items()):
                dec = self.tokenizer(description, add_special_tokens=True)
                prompts_ids_neg.append(dec['input_ids'])
                prompts_mask_neg.append(dec['attention_mask'])
                prompts_to_indices_neg.append([idx] + [label_map["lm"][entity][0]])
        ...
        if 'event' in descriptions_neg.keys():
            for idx,(trigger,event) in enumerate(descriptions_neg['event'].items()):
                count = 0
                for jdx,(entity,description) in enumerate(event.items()):
                    if count == 0:
                        dec = self.tokenizer(description, add_special_tokens=True)
                        ...
                        prompts_ids_neg.append(dec['input_ids'])
                        prompts_mask_neg.append(dec['attention_mask'])
                        prompts_to_indices_neg.append([idx] + [label_map["lr"][entity][0]])
                        count+=1
                        
        if 'cls' in descriptions_neg.keys():
            for idx, (entity, description) in enumerate(descriptions_neg['cls'].items()):
                dec = self.tokenizer(description, add_special_tokens=True)
                prompts_ids_neg.append(dec['input_ids'])
                prompts_mask_neg.append(dec['attention_mask'])
                prompts_to_indices_neg.append([idx] + [label_map["lc"][entity][0]])
                if len(label_map["lc"][entity])>1:
                    print(label_map["lc"][entity])
        if 'hyper_rel' in descriptions_neg.keys():
            for idx,(trigger,relation) in enumerate(descriptions_neg['hyper_rel'].items()):
                count = 0
                for jdx,(rel,description) in enumerate(relation.items()):
                    if count == 0:
                        dec = self.tokenizer(description, add_special_tokens=True)
                        prompts_ids_neg.append(dec['input_ids'])
                        prompts_mask_neg.append(dec['attention_mask'])
                        prompts_to_indices_neg.append([idx] + [label_map["lr"][rel][0]])
                        count+=1
                    else:
                        dec = self.tokenizer(description, add_special_tokens=True)
                        prompts_ids_neg.append(dec['input_ids'])
        ...
        ins = {
            ...
            "prompts_ids":prompts_ids,
            "prompts_mask":prompts_mask,
            "prompts_to_indices_neg":prompts_to_indices_neg,
            "prompts_ids_neg":prompts_ids_neg,
            "prompts_mask_neg":prompts_mask_neg,
            "mask": mask,
            "spans": spans,
            "label_map": label_map,
            "span_to_label": span_to_label,
    ...
    def skip_consecutive_span_labels(self, data: dict) -> dict:
        bs = len(data["input_ids"])
        des_len = len(data["prompts_ids"][0])
        des_len_neg = len(data["prompts_ids_neg"][0])
        max_seq_len = max(len(input_ids) for input_ids in data["input_ids"])
        max_prompt_len = max(len(prompts_ids) for prompts_ids in data["prompts_ids"][0])
        max_prompt_len_neg = max(len(prompts_ids_neg) for prompts_ids_neg in data["prompts_ids_neg"][0])
        # batch_seq_len = min(self.max_seq_len, max_seq_len)
        batch_seq_len = min(self.max_seq_len, max_seq_len)
        batch_prompt_len = min(self.max_seq_len, max_prompt_len)
        batch_prompt_len_neg = min(self.max_seq_len, max_prompt_len_neg)
        for i in range(bs):
            data["input_ids"][i] = data["input_ids"][i][:batch_seq_len]
            data["mask"][i] = data["mask"][i][:batch_seq_len]
            assert len(data["input_ids"][i]) == len(data["mask"][i])
            ...
            for j in range(des_len_neg):
                data["prompts_ids_neg"][i][j] = data["prompts_ids_neg"][i][j][:batch_prompt_len_neg]
                data["prompts_mask_neg"][i][j] = data["prompts_mask_neg"][i][j][:batch_prompt_len_neg]
                assert len(data["prompts_ids_neg"][i]) == len(data["prompts_mask_neg"][i])
                pad_len = batch_prompt_len_neg - len(data["prompts_mask_neg"][i][j])
                data["prompts_ids_neg"][i][j] = (
                data["prompts_ids_neg"][i][j] + [self.tokenizer.pad_token_id] * pad_len
                )
                data["prompts_mask_neg"][i][j] = data["prompts_mask_neg"][i][j] + [0] * pad_len
            # # for debugging only
            # pred_spans = decode_nnw_nsw_thw_mat(data["labels"][i].unsqueeze(0))[0]
            # sorted_gold = sorted(set(tuple(x) for x in data["spans"][i]))
            # sorted_pred = sorted(set(tuple(x) for x in pred_spans))
